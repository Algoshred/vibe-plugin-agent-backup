...
import type { HostServices, StorageTarget } from "../types.js";

const LOG_SOURCE = "backup:wspace-target";
const BACKUP_FOLDER_HIERARCHY = ["backups", "vibecontrols", "agents"];

interface FolderResult { id: string; name: string; path: string }

export class WspaceStorageTarget implements StorageTarget {
  private hostServices: HostServices;
  private folderCache = new Map<string, string>();

  constructor(hostServices: HostServices) { this.hostServices = hostServices; }
  ...
  private async ensureFolderHierarchy(agentName: string): Promise<string> {
    ...
    for (const folderName of folders) {
      ...
      if (this.folderCache.has(cacheKey)) { parentFolderId = this.folderCache.get(cacheKey)!; continue; }

      const findResult = await this.hostServices.workspaceQuery<{
        getFoldersInFolder: { folders: Array<{ id: string; name: string; path: string }> };
      ...
      const existing = findResult.data?.getFoldersInFolder?.folders?.find((f) => f.name === folderName);
      if (existing) { this.folderCache.set(cacheKey, existing.id); parentFolderId = existing.id; continue; }

      const createResult = await this.hostServices.workspaceQuery<{ createFolder: FolderResult }>(
        `mutation($input:CreateFolderInput!){createFolder(input:$input){id name path}}`,
        { input: { name: folderName, parentFolderId: parentFolderId ?? null, visibility: "WORKSPACE" } });

      if (createResult.errors?.length) throw new Error(`Failed to create folder '${folderName}': ${createResult.errors[0].message}`);
  ...
  async upload(filePath: string, agentName: string, timestamp: string): Promise<{ storagePath: string; fileId?: string }> {
    if (!this.hostServices.isGatewayConfigured()) throw new Error("Gateway is not configured. Use 'vibe setup' to configure gateway auth, or switch to custom-s3 storage target.");
    ...
    const completeResult = await this.hostServices.workspaceQuery<{
      completeUpload: { fileId: string; file: { id: string; name: string; size: string } };
    ...
    if (completeResult.errors?.length) throw new Error(`Failed to complete upload: ${completeResult.errors[0].message}`);
    const fileId = completeResult.data!.completeUpload.fileId;
    logger.info(LOG_SOURCE, `Backup uploaded: fileId=${fileId}`);
    return { storagePath: `backups/vibecontrols/agents/${agentName}/${fileName}`, fileId };
  }

  async download(storagePath: string, targetPath: string): Promise<void> {
    if (!this.hostServices.isGatewayConfigured()) throw new Error("Gateway is not configured.");
    ...
    const file = filesResult.data?.getFilesInFolder?.files?.find((f) => f.name === fileName);
    if (!file) throw new Error(`Backup file not found: ${fileName}`);

    const urlResult = await this.hostServices.workspaceQuery<{ getFileDownloadUrl: string }>(
      `query($id:ID!,$expiresIn:Int){getFileDownloadUrl(id:$id,expiresIn:$expiresIn)}`, { id: file.id, expiresIn: 3600 });
    if (urlResult.errors?.length) throw new Error(`Failed to get download URL: ${urlResult.errors[0].message}`);
  ...
  async delete(storagePath: string, fileId?: string): Promise<void> {
    ...
    if (!fileId) {
      ...
      const filesResult = await this.hostServices.workspaceQuery<{
        ...
        { folderId, pagination: { limit: 200 } });
      const file = filesResult.data?.getFilesInFolder?.files?.find((f) => f.name === basename(storagePath));
      if (!file) return;
      fileId = file.id;
    }
    await this.hostServices.workspaceQuery(`mutation($id:ID!){deleteFile(id:$id)}`, { id: fileId });
  }
  ...
  async list(agentName: string): Promise<Array<{ path: string; size: number; lastModified: string }>> {
    ...
    try {
      const folderId = await this.ensureFolderHierarchy(agentName);
      const filesResult = await this.hostServices.workspaceQuery<{
        getFilesInFolder: { files: Array<{ id: string; name: string; size: string; createdAt: string }> };
  ...
  async testConnection(): Promise<{ ok: boolean; message: string }> {
    if (!this.hostServices.isGatewayConfigured()) return { ok: false, message: "Gateway is not configured. Run 'vibe setup' first." };
    try {
      const result = await this.hostServices.workspaceQuery<{ getStorageUsage: { totalSize: string; fileCount: number } }>(
        `query{getStorageUsage{totalSize fileCount}}`);
      if (result.errors?.length) return { ok: false, message: result.errors[0].message };
      return { ok: true, message: "Connected to workspace files service" };
    } catch (err) { return { ok: false, message: `Failed: ${err instanceof Error ? err.message : String(err)}` }; }
  }
}
